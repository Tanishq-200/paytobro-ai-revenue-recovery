...
import { PaymentMethod, RecoveryCase } from '../types/index.js';

export interface SimulationParams {
  transactionCount: number;
  averageAmount: number;
  failureDistribution?: {
    temporary_bank: number; // percentage
...
export interface SimulationExecutionStep {
  stage: 'Analyzing' | 'Classifying' | 'Predicting' | 'Selecting Strategy' | 'Executing Recovery' | 'Measuring Outcome';
  message: string;
...
export function generateSimulationDataset(params: SimulationParams): SimulationGenerationResult {
  ...
  const generatedCases: RecoveryCase[] = [];
  ...
  for (let i = 0; i < transactionCount; i++) {
    ...
    insertCustomer.run(
      ...
      custName,
      custEmail,
      `+9198${String(10000000 + i).slice(-8)}`,
      reliability,
...
export function executeFullRecoverySimulation(): SimulationExecutionResult {
  ...
  for (const c of pendingCases) {
    ...
    if (isSuccessful) {
      ...
      insertAudit.run(
        `AUD_SIM_${c.id}_${Date.now()}`,
        c.id,
        c.transaction_id,
        new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        'RECOVERY_SUCCEEDED',
        c.failure_category_label,
        c.confidence_score,
        c.recommended_action_label,
        `Autonomous execution succeeded via ${c.recommended_action_label}. Probability: ${c.recovery_probability}%.`,
        'RECOVERED'
      );

      insertActivity.run(
        `ACT_SIM_${Date.now()}_${recoveredCount}`,
        'Just now',
        c.transaction_id,
        'RECOVERED',
        `₹${Number(c.amount).toLocaleString('en-IN')} successfully recovered`,
        c.amount,
  ...
  const steps: SimulationExecutionStep[] = [

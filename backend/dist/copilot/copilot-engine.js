...
export function handleCopilotQuery(query) {
    ...
    if (txMatch && (normalized.includes('why') || normalized.includes('marked') || normalized.includes('show') || normalized.includes('explain'))) {
        ...
        const row = db.prepare('SELECT * FROM recovery_cases WHERE transaction_id = ?').get(txId);
        if (row) {
            return {
                id: `msg_${Date.now()}`,
                sender: 'assistant',
    ...
    if (normalized.includes('recover first') ||
    ...
    if (normalized.includes('why did this payment fail') ||
    ...
    if (normalized.includes('potentially be recovered') ||

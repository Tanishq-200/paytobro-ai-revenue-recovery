...
export function seedDatabase(force: boolean = false): void {
  ...
  const countRow = db.prepare('SELECT count(*) as count FROM recovery_cases').get() as { count: number };
  ...
  for (const rc of rawCases) {
    ...
    insertCase.run(
      ...
      3,
      '2026-08-27 10:32:00',
      isRecovered ? rc.recoveredAt! : '2026-08-27 10:32:00',
      isRecovered ? (rc.recoveredAt || null) : null,
      isRecovered ? (rc.recoveredAmount ?? null) : null
    );
    ...
    insertAudit.run(
      ...
      decision.confidence_score,
      decision.recommended_action_label,
      `AI identified ${decision.failure_category_label} with ${decision.confidence_score}% confidence.`,
      'CLASSIFIED'
    );
    ...
    insertAudit.run(
      ...
      decision.confidence_score,
      decision.recommended_action_label,
      `Recovery probability calculated at ${decision.recovery_probability}%. Recommended strategy: ${decision.recommended_action_label}`,
      'RECOMMENDED'
    );
    ...
    if (isRecovered) {
      insertAudit.run(
        ...
        decision.confidence_score,
        decision.recommended_action_label,
        `Recovery executed in safe simulation mode. Captured ₹${rc.amount.toLocaleString('en-IN')}.`,
        'RECOVERED'
      );
    }
  }

...
apiRouter.get('/high-value', (req, res) => {
...
apiRouter.get('/cases', (req, res) => {
    const { search = '', status = '', failure_category = '', priority = '', sort_by = 'priority_score', sort_dir = 'desc', page = '1', limit = '10', } = req.query;
...
apiRouter.post('/cases/:id/execute', (req, res) => {
    const { id } = req.params;
    const row = db.prepare('SELECT * FROM recovery_cases WHERE id = ? OR transaction_id = ?').get(id, id);
    if (!row) {
        return res.status(404).json({ error: 'Case not found' });
    }
    let timeline = [];
  ...
  `).run(now, now, JSON.stringify(timeline), row.id);
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(`AUD_${row.id}_${Date.now()}`, row.id, row.transaction_id, new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), 'SIMULATION_EXECUTED', row.failure_category_label, row.confidence_score, row.recommended_action_label, `Operator executed simulated recovery via ${row.recommended_action_label}. Test payment processed.`, 'RECOVERED');
    // Add activity item
    db.prepare(`
    INSERT INTO activity_feed (id, timestamp, transaction_id, event_type, description, amount, confidence, badge_color)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(`ACT_${Date.now()}`, 'Just now', row.transaction_id, 'RECOVERED', `₹${Number(row.amount).toLocaleString('en-IN')} successfully recovered`, row.amount, row.recovery_probability, 'green');
    res.json({
        success: true,
...
apiRouter.post('/cases/:id/reject', (req, res) => {
  ...
  `).run(row.id);
    ...
    INSERT INTO audit_logs (id, case_id, transaction_id, timestamp, event_type, failure_category, confidence, recommended_action, rationale, outcome)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(`AUD_REJ_${row.id}_${Date.now()}`, row.id, row.transaction_id, new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), 'RECOVERY_REJECTED', row.failure_category_label, row.confidence_score, row.recommended_action_label, 'Operator manually rejected the AI recovery recommendation.', 'REJECTED');
...
apiRouter.get('/audit-logs', (req, res) => {
    const { case_id = '', event_type = '' } = req.query;
...
// 10. Copilot Query
apiRouter.post('/copilot/chat', (req, res) => {
    const { message = '' } = req.body;
    const reply = handleCopilotQuery(message);
    res.json(reply);
});
// 11. Simulation Lab Endpoints
apiRouter.post('/simulation/generate', (req, res) => {
...
apiRouter.post('/simulation/execute', (req, res) => {
...
apiRouter.get('/settings', (req, res) => {
    const guardrailsRow = db.prepare("SELECT value FROM settings WHERE key = 'guardrails'").get();
    const aiConfigRow = db.prepare("SELECT value FROM settings WHERE key = 'ai_config'").get();
    res.json({

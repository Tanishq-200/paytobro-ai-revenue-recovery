...
export function evaluatePaymentRecovery(params) {
    const { failureReason, failureCode, amount, paymentMethod, attemptsCount, customerReliability, config = defaultGuardrails, } = params;
    ...
    const guardrailCheck = evaluateGuardrails(classification.category, attemptsCount, scoring.recovery_probability, config);
    // 4. Select best action and draft human-readable explanation
    let recommended_action;
    let recommended_action_label;
    let action_delay_hours;
    let ai_explanation = '';
